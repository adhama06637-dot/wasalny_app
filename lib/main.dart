...
import 'package:location/location.dart'; // مكتبة الـ GPS
...
class _MapScreenState extends State<MapScreen> {
  ...
  void initState() {
    super.initState();
    _loadRoute();
    _initLocation(); // تشغيل الـ GPS أول ما الشاشة تفتح
  }

  @override
  void dispose() {
    _locationSubscription?.cancel(); // نقفل الـ GPS لما نخرج من الشاشة عشان البطارية
    super.dispose();
  }

  // 🌟 دالة تشغيل وإذن الـ GPS 🌟
  Future<void> _initLocation() async {
    ...
    // أول ما ياخد الإذن، يجيب مكاني الحالي
    _currentLocation = await _location.getLocation();
    if (mounted) setState(() {});
  }

  // 🌟 دالة بدء الرحلة (Start Trip) زي أوبر 🌟
  void _startTrip() {
    setState(() {
      _isTracking = true;
    });

    // الخريطة تقرب عليك بزووم عالي
    if (_currentLocation != null) {
      _mapController.move(
        LatLng(_currentLocation!.latitude!, _currentLocation!.longitude!),
        17.0, // مستوى الزووم زي أوبر
      );
    }

    // يبدأ يراقب حركتك وكل ما تمشي الخريطة تمشي معاك
    _locationSubscription = _location.onLocationChanged.listen((LocationData loc) {
      if (mounted) {
        setState(() {
          _currentLocation = loc;
        });
        // لو التتبع شغال، خلي الكاميرا دايماً متسنترة عليك
        if (_isTracking) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Trip Started! GPS tracking activated 🚀'),
      ...
      body: Stack(
        children: [
          if (!isLoading && errorMessage == null)
            FlutterMap(
              mapController: _mapController, // 🌟 ربطنا الخريطة بالكنترولر
              options: MapOptions(
                initialCenter: _getCenter(),
                initialZoom: 12,
                // عشان لو اليوزر حرك الخريطة بإيده التتبع يقف مؤقتاً وميضايقوش
              ...
              children: [
                ...
                if (routePoints.isNotEmpty)
                  PolylineLayer(
                    polylines: [
                      Polyline(points: routePoints, strokeWidth: 5, color: const Color(0xFF00E676)),
                    ],
                  ),
                MarkerLayer(
                  markers: [
                    if (startPoint != null)
                      Marker(point: startPoint!, width: 40, height: 40, child: const Icon(Icons.location_on, color: Colors.blue, size: 40)),
                    if (endPoint != null)
                      Marker(point: endPoint!, width: 40, height: 40, child: const Icon(Icons.flag, color: Colors.red, size: 40)),
                    
                    // 🌟 النقطة الزرقاء بتاعت اليوزر (زي أوبر) 🌟
                    if (_currentLocation != null)
                      Marker(
          ...
          // 🌟 زرار إعادة التمركز لو اليوزر حرك الخريطة بعيد 🌟
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        // 🌟 هنا بننده على دالة بدء الرحلة 🌟
                        onPressed: _isTracking ? null : _startTrip,
                        style: ElevatedButton.styleFrom(
                          backgroundColor: _isTracking ? Colors.grey : const Color(0xFF00E676),
                          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)),
                        ),
                        child: Text(_isTracking ? 'Tracking Active 📍' : 'Start Trip', 
